...
- 垂直方向的距离由margin决定， 属于同一个`BFC`的两个相邻的上下标签外边距会发生重叠，解决margin重叠问题
...
Node.js不为每个客户连接创建一个新的线程，而仅仅使用**一个线程**。当有用户连接了，就触发一个**内部事件**，通过非阻塞I/O、事件驱动机制，让Node.js程序宏观上也是并行的。使用Node.js，一个8GB内存的服务器，可以同时处理超过4万用户的连接。
